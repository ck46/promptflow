...
import re
from typing import List, Dict, Any, Optional, Pattern, Set, Union
import tiktoken
...
from promptflow.prompt_filters.base import PromptFilter, FilterResult
            ...
            for keyword in self.keywords:
                ...
                if search_keyword in content:
                    return FilterResult(
                        ...
                        details={"keyword": keyword, "message_role": message.role}
                    )
        ...
        for message in prompt.messages:
            ...
            for pattern in self.patterns:
                ...
                if match:
                    return FilterResult(
                        passed=False,
                        reason=f"Prompt matches banned pattern: {pattern.pattern}",
                        details={
...
class MaxTokenFilter(PromptFilter):
    ...
    def check(self, prompt: Prompt) -> FilterResult:
        ...
        if token_count > self.max_tokens:
            return FilterResult(
                passed=False,
                reason=f"Prompt exceeds maximum token count: {token_count} > {self.max_tokens}",
