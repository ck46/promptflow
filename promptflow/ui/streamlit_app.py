...
def load_versions(prompt_name):
    ...
    if st.session_state.flow and prompt_name:
        st.session_state.versions = st.session_state.flow.list_versions(prompt_name)
        if st.session_state.versions:
            st.session_state.current_version = st.session_state.versions[-1]  # Latest version
...
def create_new_prompt(name, system_message, user_message, category, tags, description):
    """Create a new prompt."""
    if st.session_state.flow:
        # Create prompt
...
def test_prompt(prompt, test_input):
    ...
    has_user_message = test_input or any(msg.role == MessageRole.USER for msg in builder._messages)
    ...
    return f"This is where the LLM response would appear. The prompt has {len(test_prompt.messages)} messages."
...
def render_sidebar():
    ...
    if not st.session_state.initialized:
        ...
        page = st.sidebar.radio("Select Page", ["Prompt Management", "Prompt Testing", "Settings"])
...
def render_prompt_management():
    ...
    tab1, tab2, tab3 = st.tabs(["Create Prompt", "View/Edit Prompts", "Fallback Configuration"])
    ...
    with tab1:
        ...
        with st.form("create_prompt_form"):
            ...
            system_message = st.text_area("System Message", key="new_system_message")
            ...
            category = st.selectbox(
                "Category", 
                options=[c.value for c in PromptCategory],
                key="new_category"
            )
            ...
            if submitted:
                if name:
                    ...
                    if version:
                        st.success(f"Prompt '{name}' created with version {version}")
                else:
    ...
    with tab2:
        ...
        if st.session_state.prompts:
            prompt_name = st.selectbox(
                ...
                on_change=lambda: load_versions(st.session_state.view_prompt_name)
            )
            ...
            if st.session_state.versions:
                version = st.selectbox(
                    ...
                    key="view_version",
                    on_change=lambda: load_prompt(prompt_name, st.session_state.view_version)
                )
                
                # Load the prompt if not already loaded
                if not st.session_state.current_prompt or st.session_state.current_version != version:
                ...
                if prompt:
                    ...
                    if st.button("Set as Active Version"):
                        ...
                        st.success(f"Set {prompt_name} version {version} as active")
            else:
    ...
    with tab3:
        ...
        with st.form("fallback_form"):
            ...
            if primary_prompt and fallback_prompt:
                ...
                fallback_versions = st.session_state.flow.list_versions(fallback_prompt)
                
                if fallback_versions:
                    fallback_version = st.selectbox(
                        "Fallback Version", 
                        options=fallback_versions,
                        key="fallback_version"
                    )
                    
                    submitted = st.form_submit_button("Set Fallback")
                    
                    if submitted:
                        set_fallback_prompt(fallback_prompt, fallback_version, primary_prompt)
                        st.success(f"Set {fallback_prompt} version {fallback_version} as fallback for {primary_prompt}")
                else:
    ...
    if st.session_state.prompts:
        ...
        with col1:
            prompt_name = st.selectbox(
                ...
                on_change=lambda: load_versions(st.session_state.test_prompt_name)
            )
        ...
        with col2:
            ...
            if "versions" in st.session_state and st.session_state.versions:
                ...
                use_active = st.checkbox("Use Active Version", value=True, key="use_active")
            else:
        ...
        if prompt_name:
            if use_active:
                ...
                if not prompt:
                    st.warning(f"No active version found for {prompt_name}. Using latest version.")
            ...
            if prompt:
                ...
                test_input = st.text_area("Enter test input", key="test_input_text")
                ...
                if st.session_state.test_result:
                    ...
                    if st.session_state.test_prompt:
                        with st.expander("Full Test Prompt", expanded=False):
                            for i, msg in enumerate(st.session_state.test_prompt.messages):
                                st.text_area(
                                    f"{msg.role.value.capitalize()} Message {i+1}", 
                                    value=msg.content,
...
def render_settings():
    ...
    if st.session_state.flow:
        st.write(f"**Current Database URL:** {st.session_state.flow.repo.db_url}")
    
    # Option to change database path
    new_db_path = st.text_input("New Database Path (relative to current directory)")
