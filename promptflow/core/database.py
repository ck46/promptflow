...
class PromptModel(Model):
    ...
    @classmethod
    def from_prompt(cls, name: str, version: str, prompt: Prompt) -> "PromptModel":
        ...
        return cls(
            ...
            parameters_json=json.loads(json.dumps(prompt.parameters.dict())) if prompt.parameters else {},
...
class DBPromptRepo:
    ...
    async def init(self):
        ...
        if not self._is_initialized:
            await Tortoise.init(
                db_url=self.db_url,
                modules={'models': ['promptflow.core.database']}
            )
    ...
    async def save_prompt(self, prompt_name: str, prompt: Prompt, 
        ...
        if prompt_model:
            ...
            prompt_model.messages = json.loads(json.dumps([m.dict() for m in prompt.messages]))
            prompt_model.metadata_json = json.loads(json.dumps(prompt.metadata.dict())) if prompt.metadata else {}
            prompt_model.parameters_json = json.loads(json.dumps(prompt.parameters.dict())) if prompt.parameters else {}
            prompt_model.stats_json = json.loads(json.dumps(prompt.stats.dict())) if prompt.stats else {}
            ...
            # Create new prompt
            prompt_model = PromptModel.from_prompt(prompt_name, version, prompt)
            await prompt_model.save()
        
        return version
    
    async def get_prompt(self, prompt_name: str, version: Optional[str] = None) -> Optional[Prompt]:
        """
    ...
    async def get_active_prompt(self, prompt_name: str) -> Optional[Prompt]:
        ...
        prompt_model = await PromptModel.filter(
            name=prompt_name, 
            is_active=True
    ...
    async def list_prompts(self, category: Optional[PromptCategory] = None) -> List[str]:
        """
    ...
    async def list_versions(self, prompt_name: str) -> List[str]:
        ...
        prompt_models = await PromptModel.filter(name=prompt_name).order_by('version').values('version')
    ...
    async def set_active(self, prompt_name: str, version: str, active: bool = True) -> None:
        """
        ...
        if active:
            await PromptModel.filter(name=prompt_name).exclude(version=version).update(is_active=False)
    
    async def set_fallback(self, prompt_name: str, version: str, fallback_for: str) -> None:
        """
    ...
    async def update_stats(self, prompt_name: str, version: str, success: bool = True) -> None:
        """
