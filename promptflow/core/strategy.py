...
class PromptStrategy(ABC):
    ...
    @abstractmethod
    async def select_prompt(self, prompt_name: str, context: Dict[str, Any] = None) -> Optional[Prompt]:
        """
...
class ConditionalPromptStrategy(PromptStrategy):
    ...
    def __init__(self, repo: DBPromptRepo, condition_fn: Callable[[Dict[str, Any]], bool], 
                 if_true: PromptStrategy, if_false: PromptStrategy):
        """
        Initialize the strategy.
...
class ABTestingPromptStrategy(PromptStrategy):
    ...
    def __init__(self, repo: DBPromptRepo, prompt_variants: List[str], weights: Optional[List[float]] = None):
        """
        Initialize the strategy.
    ...
    async def select_prompt(self, prompt_name: str, context: Dict[str, Any] = None) -> Optional[Prompt]:
        ...
        variant_name = self.random.choices(self.prompt_variants, weights=self.weights, k=1)[0]
...
class ContextAwarePromptStrategy(PromptStrategy):
    ...
    def __init__(self, repo: DBPromptRepo, context_key: str, prompt_mapping: Dict[Any, str]):
        """
        Initialize the strategy.
...
class PromptSelector:
    ...
    def __init__(self, repo: DBPromptRepo, default_strategy: PromptStrategy = None):
        """
        ...
        self.fallback_strategy = FallbackPromptStrategy(repo, self.active_strategy)
        self.latest_strategy = LatestPromptStrategy(repo)
    
    async def select_prompt(self, prompt_name: str, strategy: Optional[PromptStrategy] = None, 
    ...
    def create_ab_testing_strategy(self, prompt_variants: List[str], 
                              weights: Optional[List[float]] = None) -> ABTestingPromptStrategy:
        """
    ...
    def create_context_aware_strategy(self, context_key: str, 
                                 prompt_mapping: Dict[Any, str]) -> ContextAwarePromptStrategy:
        """
        ...
        """
        return ContextAwarePromptStrategy(self.repo, context_key, prompt_mapping)
    
    def create_conditional_strategy(self, condition_fn: Callable[[Dict[str, Any]], bool], 
                               if_true: PromptStrategy, 
                               if_false: PromptStrategy) -> ConditionalPromptStrategy:
        """
        ...
        """
        return ConditionalPromptStrategy(self.repo, condition_fn, if_true, if_false)
    
    def create_category_strategy(self, category: PromptCategory) -> CategoryPromptStrategy:
        """
