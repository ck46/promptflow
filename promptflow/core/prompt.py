...
class Prompt(BaseModel):
    ...
    def add_message(self, role: Union[str, MessageRole], content: str, name: Optional[str] = None) -> "Prompt":
    ...
    def add_function_call(self, function_name: str, function_args: Dict[str, Any]) -> "Prompt":
    ...
    def add_function_definition(self, name: str, description: Optional[str] = None, 
                               parameters: Optional[Dict[str, Any]] = None) -> "Prompt":
...
class PromptBuilder:
    ...
    def add_message(self, role: Union[str, MessageRole], content: str, name: Optional[str] = None) -> "PromptBuilder":
        """Add a message to the prompt."""
        if isinstance(role, str):
            role = MessageRole(role)
    ...
    def add_function_call(self, function_name: str, function_args: Dict[str, Any]) -> "PromptBuilder":
        """Add a function call message to the prompt."""
        return self.add_message(
            MessageRole.FUNCTION, 
            json.dumps(function_args),
    ...
    def add_function_definition(self, name: str, description: Optional[str] = None, 
                              parameters: Optional[Dict[str, Any]] = None) -> "PromptBuilder":
        """Add a function definition to the prompt parameters."""
        if self._parameters is None:
            self._parameters = PromptParameters()
    ...
    def build(self, require_user_message: bool = False) -> Prompt:
        ...
        if require_user_message:
            has_user_message = any(msg.role == MessageRole.USER for msg in self._messages)
            if not has_user_message:
                raise ValueError("Prompt must contain at least one user message when require_user_message=True")
