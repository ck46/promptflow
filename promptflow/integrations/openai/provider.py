...
from promptflow.core.response import LLMResponse, StreamingResponse, FunctionCall
...
class OpenAIProvider(LLMProvider):
    ...
    def _convert_prompt_to_messages(self, prompt: Prompt) -> List[Dict[str, Any]]:
        """
    ...
    def _parse_streaming_chunk(self, chunk: Dict[str, Any], stream_response: StreamingResponse) -> None:
        """
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=body) as resp:
                if resp.status != 200:
                    ...
                    raise ValueError(f"OpenAI API error ({resp.status}): {error_text}")
