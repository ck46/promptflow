...
from promptflow.prompt_filters.base import FilterResult
...
def test_keyword_filter():
    ...
    prompt1 = PromptBuilder()\
        .add_user("How can I improve my programming skills?")\
        .build()
    ...
    filter_case_sensitive = KeywordFilter(
        keywords=["Hack", "Illegal", "Bomb"],
        case_sensitive=True
    )
    
    # This shouldn't match because the case is different
    prompt3 = PromptBuilder()\
        .add_user("How can I hack into a website?")\
        .build()
...
def test_regex_filter():
    ...
    prompt1 = PromptBuilder()\
        .add_user("What is the weather like today?")\
        .build()
    
    result1 = filter_obj.check(prompt1)
    assert result1.passed is True
    
    # Test a prompt that matches the credit card pattern
    prompt2 = PromptBuilder()\
        .add_user("What is my credit card number?")\
        .build()
    
    result2 = filter_obj.check(prompt2)
    assert result2.passed is False
    assert "credit" in result2.reason
    
    # Test a prompt that matches the email pattern
    prompt3 = PromptBuilder()\
        .add_user("My email is user@example.com")\
        .build()
    
    result3 = filter_obj.check(prompt3)
    assert result3.passed is False
    assert "user@example.com" in result3.details["matched_text"]
    
    # Test a prompt that matches the phone number pattern
    prompt4 = PromptBuilder()\
        .add_user("Call me at 123-456-7890")\
        .build()
    
    result4 = filter_obj.check(prompt4)
    assert result4.passed is False
    assert "123-456-7890" in result4.details["matched_text"]
...
def test_filter_pipeline():
    ...
    max_token_filter = MaxTokenFilter(
        max_tokens=100,
        name="TokenLimitFilter"
    )
    
    # Create a pipeline
    pipeline = FilterPipeline([keyword_filter, max_token_filter])
    ...
    assert result1.passed is True
    assert "filter_results" in result1.details
    assert all(res["passed"] for res in result1.details["filter_results"].values())
    ...
    # Test a prompt that fails the token limit filter
    long_text = "This is a very long prompt that should exceed the token limit. " * 20
    prompt3 = PromptBuilder()\
        .add_user(long_text)\
        .build()
    ...
    # Test adding a filter to the pipeline
    regex_filter = RegexFilter(
        patterns=[r"credit\s*card"],
        name="FinancialInfoFilter"
    )
    
    pipeline.add_filter(regex_filter)
    
    # Test a prompt that fails the new filter
    prompt4 = PromptBuilder()\
        .add_user("What is my credit card number?")\
        .build()
    
    result4 = pipeline.check(prompt4)
    assert result4.passed is False
    assert result4.details["failed_filter"] == "FinancialInfoFilter" 